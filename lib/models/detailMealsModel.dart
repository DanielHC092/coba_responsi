...
class MealResponse {
  ...
  MealResponse({required this.meal});

  factory MealResponse.fromJson(Map<String, dynamic> json) {
    var list = json['meals'] as List; // Ensure this matches the API response
    List<Meal> mealList = list.map((i) => Meal.fromJson(i)).toList();
    return MealResponse(meal: mealList);
  }
}
